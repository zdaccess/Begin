...
public class Program {
    public static void main(String[] args) {
        ...
        while (!dataName1.equals(".") && i != 10)
        {
            ...
            if ((wer[i][0] <= '1' || wer[i][0] >= '7') || (!month[i][1].equals("TU") && !month[i][1].equals("WE") &&
                    !month[i][1].equals("TH") && !month[i][1].equals("FR") && !month[i][1].equals("SA") && !month[i][1].equals("SU") && !month[i][1].equals("MO")))
                i--;
            else {
                j = 0;
                flag = 0;
                while (j < i + 1)
                {
                    if (month[j][0].equals(month[i][0]) && month[j][1].equals(month[i][1])) {
                        flag++;
                        if (flag == 2) {
                            i--;
                            break;
        ...
        while (!dataName1.equals("."))
        {
            ...
            while (j < s + 1)
            {
                if (shedule[j][0].equals(shedule[s][0]) && shedule[j][1].equals(shedule[s][1]) && shedule[j][2].equals(shedule[s][2]) && shedule[j][3].equals(shedule[s][3])) {
                    flag++;
                    if (flag == 2) {
                        s--;
                        break;
        ...
        while (i < nameLenght)
        {
            ...
            {
                if (name[i] == null)
                    System.out.print(days[j][0] +  ":00 " + days[j][1] + "    " + days[j][2] + "|");
                else
                {
                    flag = 0;
                    s = 0;
                    while (s < sheduleLenght) {
                        if (name[i].equals(shedule[s][0]) && days[j][0].equals(shedule[s][1]) && days[j][2].equals(shedule[s][2]) && shedule[s][3].equals("NOT_HERE") && days[j][2].length() == 1)
                        {
                            flag = 1;
                            System.out.print("          -1|");
                            break;
                        }
                        else if (name[i].equals(shedule[s][0]) && days[j][0].equals(shedule[s][1]) && days[j][2].equals(shedule[s][2]) && shedule[s][3].equals("NOT_HERE") && days[j][2].length() > 1)
                        {
                            flag = 1;
                            System.out.print("           -1|");
                            break;
                        }
                        else if (name[i].equals(shedule[s][0]) && days[j][0].equals(shedule[s][1]) && days[j][2].equals(shedule[s][2]) && shedule[s][3].equals("HERE") && days[j][2].length() == 1)
                        {
                            flag = 1;
                            System.out.print("           1|");
                            break;
